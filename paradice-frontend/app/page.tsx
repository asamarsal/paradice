        ...
        <section className="w-full rounded-[1.5rem] border border-white/10 bg-white/5 p-4 shadow-2xl backdrop-blur-md">
          ...
          {state.phase === "game_over" && (
            <>
              ...
              {playerWon && (
                <div className="mt-3 rounded-[1.25rem] border border-amber-300 bg-amber-50 px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.25em] text-amber-700">Winner NFT</p>
                      <p className="mt-1 text-xs font-semibold text-amber-900/80">Klaim NFT pemenang untuk match ini.</p>
                    </div>
                    <button
                      onClick={() => void handleClaimWinnerNft()}
                      disabled={winnerNftClaimed || isClaimingWinnerNft}
                      className="rounded-full border border-amber-400 bg-amber-100 px-4 py-2 text-[10px] font-black uppercase tracking-[0.15em] text-amber-800 transition hover:bg-amber-200 disabled:cursor-not-allowed disabled:opacity-70"
                    >
                      {winnerNftClaimed ? "Claimed" : isClaimingWinnerNft ? "Claiming..." : "Claim NFT"}
                    </button>
                  </div>
                  {winnerNftClaimNotice && (
                    <p className="mt-3 rounded-lg border border-amber-200 bg-white/80 px-3 py-2 text-xs font-semibold text-amber-900">
                      {winnerNftClaimNotice}
                    </p>
                  )}
                </div>
