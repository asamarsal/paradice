...
export default function ProfilePage() {
    const [dummyBalanceUsd, setDummyBalanceUsd] = useState(20.00);
    ...
    return (
        <div className="relative min-h-screen overflow-hidden bg-[#0f0c1a] font-sans">
            ...
            {/* Back button */}
            <div className="relative z-20 mx-auto max-w-7xl px-4 py-8 md:px-8 pb-3">
                <Link
                    href="/"
                    className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-xs font-black uppercase tracking-widest text-white/70 backdrop-blur transition hover:bg-white/15 hover:text-white"
                >
                ...
                <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
                    ...
                    <div className="flex flex-col gap-4">
                                    ...
                                    <div className={`mt-3 inline-flex items-center gap-2 rounded-full bg-gradient-to-r ${tier.bg} px-3 py-1 shadow-lg`}>
                                        ...
                                        <div className="flex gap-0.5">
                                            {Array.from({ length: 5 }).map((_, i) => (
                                                <span key={i} className={`text-[10px] ${i < tier.stars ? 'text-[#1a0f2e]' : 'text-[#1a0f2e]/20'}`}>★</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="flex gap-0 overflow-hidden rounded-xl border border-white/10 bg-white/[0.04] backdrop-blur">
                            {['STATS', 'INVENTORY', 'FRIENDS', 'HISTORY'].map((tab) => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`flex-1 py-3 text-[11px] font-black uppercase tracking-widest transition ${activeTab === tab
                        ...
                        {activeTab === 'STATS' && (
                            <div className="flex flex-col gap-4">
                                <div className="grid grid-cols-3 gap-3">
                                    {[
                                        { icon: '▶', label: 'Matches', value: user.matches, color: 'text-emerald-400' },
                                        { icon: '🏆', label: 'Wins', value: user.wins, color: 'text-yellow-400' },
                                        { icon: '⚡', label: 'Winrate', value: `${user.winrate}%`, color: 'text-orange-400' },
                                    ].map((stat) => (
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="rounded-xl border border-white/10 bg-white/[0.06] p-4 backdrop-blur">
                                        <div className="text-[10px] font-bold uppercase tracking-widest text-white/40">Total Earnings</div>
                                        <div className="mt-1 text-xl font-black text-emerald-400">${user.totalEarnings.toFixed(2)}</div>
                                    </div>
                                    <div className="rounded-xl border border-white/10 bg-white/[0.06] p-4 backdrop-blur">
                                        <div className="text-[10px] font-bold uppercase tracking-widest text-white/40">Win Streak</div>
                                        <div className="mt-1 text-xl font-black text-orange-400">{user.winStreak} Wins 🔥</div>
                                    </div>
                                </div>

                                <div className="rounded-xl border border-white/10 bg-white/[0.06] p-5 backdrop-blur">
                                    <div className="mb-4 flex items-center justify-between">
                                        <span className="text-[11px] font-black uppercase tracking-widest text-white/60">Friends</span>
                                        <Link href="/leaderboard">
                                            <button className="text-[10px] font-black uppercase tracking-widest text-orange-400 transition hover:text-orange-300">View All &rsaquo;</button>
                                        </Link>
                                    </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-6 backdrop-blur-xl shadow-2xl">
                        <h2 className="mb-6 text-lg font-black text-white">Update Profile</h2>
                        <div className="flex flex-col gap-5">
                            <div>
                                <label className="mb-1.5 block text-xs font-black uppercase tracking-widest text-white/50">Username</label>
                            ...
                            <div>
                                <label className="mb-1.5 block text-xs font-black uppercase tracking-widest text-white/50">Bio</label>
                            ...
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className={`h-12 w-full rounded-xl text-sm font-black uppercase tracking-[0.15em] shadow-lg transition active:scale-95 ${saved
                                    ? 'bg-emerald-500 text-white shadow-emerald-500/30'
                                    : saving
                                        ? 'cursor-not-allowed bg-[#F97316]/60 text-white/60'
                                        : 'bg-gradient-to-r from-[#F97316] to-[#EAB308] text-[#1a0f2e] shadow-orange-500/30 hover:-translate-y-px hover:shadow-orange-400/50'
                                    }`}
                            >
